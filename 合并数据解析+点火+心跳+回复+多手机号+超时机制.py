...
# 全局超时记录：记录每个设备最近一次发送到北斗 API 的时间戳和超时时长（秒）
last_sent_info = {}
# 配置日志
    ...
    try:
        ...
        if response.status_code == 200:
            ...
            if response_data.get("code") == 0:
                ...
                last_sent_info[phone_number] = (time.time(), 15 * 60)
            else:
...
def handle_query_smoke_state(phone_number):
    ...
    try:
        ...
        if response.status_code == 200:
            ...
            last_sent_info[phone_number] = (time.time(), 5 * 60)
        else:
...
def handle_load_smokestick(phone_number):
    ...
    try:
        ...
        if response.status_code == 200:
            ...
            last_sent_info[phone_number] = (time.time(), 5 * 60)
            logging.info(f"[{phone_number}] 已记录发送时间，1 分钟内有效")
        else:
...
def process_content(content, phone_number):
    ...
    try:
        ...
        logging.error(f"[{phone_number}] ASCII 解码失败: {e}")
        return None
    logging.info(f"接收到的ASCII数据: {received_ascii}")

    # 校验 header
    header = received_ascii[:8]  # 取前 8 字符
    if header != "$#0001TB":
        logging.warning(f"[{phone_number}] 非预期 header={header}，丢弃")
        return None
...
def receive_data():
    """
    增加超时机制：只有在 handle_xxx() 成功发送后的超时时间内的接收才会处理，超时丢弃。
    """
    data = request.json
    ...
    if 'commInfos' in data:
        for item in data['commInfos']:
            content = item.get('content')     # 原始 HEX 字符串（带空格）
            from_addr = item.get('fromAddr')  # 设备卡号 (8 位)
            ...
            # 超时检查
            record = last_sent_info.get(phone_number)
            if not record:
                logging.warning(f"[{phone_number}] 未检测到发送记录，丢弃接收")
                continue
            sent_time, timeout_secs = record
            elapsed = time.time() - sent_time
            if elapsed > timeout_secs:
                logging.warning(
                    f"[{phone_number}] 回调超时 {elapsed/60:.1f} 分钟（要求 {timeout_secs/60:.1f} 分钟内），丢弃"
                )
                continue
